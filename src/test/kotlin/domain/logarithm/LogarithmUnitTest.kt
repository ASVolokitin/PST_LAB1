...
class LogarithmUnitTest {
    ...
    @ParameterizedTest(name = "x = {0}, expected log_2(x) = {1}")
    @CsvFileSource(resources = ["/log2_x_data.csv"], numLinesToSkip = 1)
    fun shouldCalculateLogarithmUsingCsvData(
        ...
        val mockedLnBase = BigDecimal("0.6931471805599453")
        val mockedLnX = expectedLogValue.multiply(mockedLnBase)
